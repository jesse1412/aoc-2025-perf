pub fn run(input: &str) -> i64 {
    ...
    for (y, row) in grid.into_iter().enumerate() {
        for (x, c) in row.into_iter().enumerate() {
            if c == '@' {
                ...
                add_in_9x9(y_limit, x_limit, &mut adjacent_counts, y, x, true);
            };
        }
    }
    let mut tot = 0;
    ...
    while found_one {
        let mut new_roll_positions = Vec::with_capacity(roll_positions.len());
        found_one = false;
        for (y, x) in roll_positions {
            if adjacent_counts[y][x] < 4 {
                tot += 1;
                found_one = true;
                add_in_9x9(y_limit, x_limit, &mut adjacent_counts, y, x, false);
            } else {
                new_roll_positions.push((y, x));
            }
        }
...
fn add_in_9x9(
    ...
    for y_offset in -1..2 {
        for x_offset in -1..2 {
            ...
            if plus {
                adjacent_counts[y as usize][x as usize] += 1;
            } else {
                adjacent_counts[y as usize][x as usize] =
                    adjacent_counts[y as usize][x as usize].saturating_sub(1);
            }
        }
    }
}
